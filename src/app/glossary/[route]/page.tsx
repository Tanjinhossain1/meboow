...
import { fetchArticles, fetchGlossary, fetchGlossaryList } from '@/services/articleServices'
...
export default async function page({ params }: { params: { route: string } }) {
  ...
  return (
   <>
   <Navbar />
   <MainComponent glossaryList={glossaryList.data} latestArticles={MobilesArticles?.data} glossary={glossary.data && glossary.data[0] ? glossary.data[0] : null } />
   <Footer />
   </>
  )
}
